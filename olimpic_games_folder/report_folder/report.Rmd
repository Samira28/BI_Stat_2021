...
olympic_func <- function(path, format) {
   ...
   if (format == "csv") olympic_data1 <- {list.files(path, full.names = TRUE) %>% lapply(read_csv) %>% bind_rows} else{
       if (format == "txt") olympic_data1 <- {list.files(path, full.names = TRUE) %>% lapply(read_txt) %>% bind_rows}
       if (format == "xlsx") olympic_data1 <- {list.files(path, full.names = TRUE) %>% lapply(read_xlsx) %>% bind_rows}
     }
...
```{r}
olympic_data <- olympic_func(path = "/home/samira/Samira/athlete_events/athlete_events/", format = "csv")
```
...
```{r}
olympic_data$Team <- gsub('[0-9,-]+', '', olympic_data$Team)
...
```{r}
olymp_data_winter <- subset(olympic_data, Season == "Winter")
```
...
After conducting a statistical analysis, we can conclude that the average height and weight in men and women differ, that cannot be said about age.
